function [M,m,A_s,A_b] = Mass(D,I_D,n_s,n_b,s,b,C_sT,C_bT,rho_s,rho_b,m_s_total,m_b_total,m_load,q,r)

% To calculate the cross section area of each string and bar
Total_length_of_strings = 0;
Total_length_of_bars = 0;
for i = 1:n_s
    Total_length_of_strings = Total_length_of_strings + sqrt( s(1+D*(i-1))^2 + s(2+D*(i-1))^2 );
end
for i = 1:n_b
    Total_length_of_bars = Total_length_of_bars + sqrt( b(1+D*(i-1))^2 + b(2+D*(i-1))^2 );
end
