...
BarWidth = 1.7; % Width of bar lines
StringWidth = 0.8; % Width of string lines
...
if ~isempty(C_b)
    ...
    if ~isempty(R3Ddata) && isfield(R3Ddata,'Bradius') % 3D plot
        ...
        quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'blue.','Autoscale','off','LineWidth',BarWidth)
        for j = 1:size(B,2)
        plot3([bar_start_nodes(1,j),bar_end_nodes(1,j)],...
            [bar_start_nodes(2,j),bar_end_nodes(2,j)],...
            [bar_start_nodes(3,j),bar_end_nodes(3,j)],...
            'Color',BarSurfColor,'LineWidth',BarWidth);
        hold on
        end
