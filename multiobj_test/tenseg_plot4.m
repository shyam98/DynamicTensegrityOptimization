function [fig_out] = tenseg_plot4( N,C_b,C_s,fig_handle,highlight_nodes,view_vec, PlotTitle, R3Ddata)
...
%% 3D plot options
nsurfpatches = 5; % Number of surface patches in 3D plots
...
StringSurfColor1 = [0.6, 0.15, 0.15];
StringSurfColor2 = [1, 0.1, 0.1];
...
if ~isempty(C_b)
    ...
    if ~isempty(R3Ddata) && isfield(R3Ddata,'Bradius') % 3D plot
        ...
        %quiver3(bar_start_nodes(1,:),bar_start_nodes(2,:),bar_start_nodes(3,:),B(1,:),B(2,:),B(3,:),'.','Color',BarSurfColor,'Autoscale','off','LineWidth',BarWidth)
        for j = 1:size(B,2)
            MyMinVal = min(N(3,:)); MyMaxVal = max(N(3,:)); 
            eta = (0.5*(bar_start_nodes(3,j)+bar_end_nodes(3,j)) - MyMinVal)/(MyMaxVal-MyMinVal);
            MyColor = BarSurfColor1 + eta*(BarSurfColor2 - BarSurfColor1);
        plot3([bar_start_nodes(1,j),bar_end_nodes(1,j)],...
            [bar_start_nodes(2,j),bar_end_nodes(2,j)],...
            [bar_start_nodes(3,j),bar_end_nodes(3,j)],...
            'Color',MyColor,'LineWidth',BarWidth);
        hold on
        end
...
str = '$x$ [m]'; xlabel(str,'Interpreter','latex')
str = '$y$ [m]'; ylabel(str,'Interpreter','latex')
str = '$z$ [m]'; zlabel(str,'Interpreter','latex')

title(PlotTitle)
set(gca,'FontName','Times New Roman','fontsize', 30,'linewidth',1.15)
